...
  body: [{
    name,
    video_link: video_link || null: || null,
    notes: null,
    is_system: false,
...
          const card = document.createElement("div");
          card.className = "item";
          card.innerHTML = `<h3>${e.name}</h3><div class="small">${e.primary_muscle || ""} - ${e. || ""}</div>${renderVideoThumb(e.video_link)}`;
          list.appendChild(card);
        });
        if (!ex || ex.length === 0) list.innerHTML = `<div class="muted">No exercises found.</div>`;
      }
...
      const card = document.createElement("div");
      card.className = "item";
      card.innerHTML = `<h3>${e.name}</h3><div class="small">${e.video_link || ""} - ${e. || ""}</div>`;
      list.appendChild(card);
    });
  } catch (err) {
    console.error(err);
