...
// Best set selection for âtop setâ style display
...
// Load video links for a set of exercise IDs (so we can show thumbnails inside an active workout)
async function loadExerciseVideoLinksByIds(exerciseIds) {
...
    chevron.textContent = isOpen ? "Hide â´" : "Show â¾";
...
    search.placeholder = "Search exercises to addâ¦";
...
        chevron.textContent = "Hide â´";
        openProgramIds.add(t.id);
      } else {
        details.classList.add("hidden");
        chevron.textContent = "Show â¾";
        openProgramIds.delete(t.id);
      }
    });
...
    renderSets();
    
// Layout: inputs on the left, clickable video thumbnail on the right (if we have a link)
const body = document.createElement("div");
body.className = "workout-exercise-body";

const left = document.createElement("div");
left.className = "workout-exercise-left";

const right = document.createElement("div");
right.className = "workout-exercise-right";
right.innerHTML = renderVideoThumb(item.video_link || "") || `<div class="muted small">No video</div>`;

left.append(setsBox, addSet);
...
const exerciseIds = (tpl.items || []).map((it) => it.exercise_id);

// Pull video links for these exercises (optional; used for thumbnails in the workout UI)
let videoByExerciseId = new Map();
try {
  const exRows = await loadExerciseVideoLinksByIds(exerciseIds);
  (exRows || []).forEach((r) => videoByExerciseId.set(r.id, r.video_link || ""));
} catch (e) {
  console.warn("Could not load exercise video links:", e);
}
...
            video_link: videoByExerciseId.get(we.exercise_id) || "",
...
          return `ð ${p.exerciseName} â ${bits.join(" | ")}`;
        });
        prMsg = "\n" + lines.join("\n");
      }
    } catch (e) {
